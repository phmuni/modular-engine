...
void createDefaultModel(Engine &engine);
...
int main() {
  ...
  createDefaultModel(engine);
  createSpotlight(engine);
  createCamera(engine);
...
void createDefaultModel(Engine &engine) {
  ...
  engine.createEntityWithModel(modelPath, texturePath, position, rotation, scale);
}
